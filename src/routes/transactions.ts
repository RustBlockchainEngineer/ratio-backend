...
import { getDetailTransactions, getVault, parseTx, parseTxsignatures, addDeposit, addWithdraw, addBorrow, addPayback, addStake, addSwap, addReward } from '../api/transactions'
...
router.get('/:wallet_id/signatures', async function (req: Request, res: Response) {
    ...
    let result = await parseTxsignatures(req.params.wallet_id, function (result) {
        res.send(JSON.stringify(result));
    });
})
