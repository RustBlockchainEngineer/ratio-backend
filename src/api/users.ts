...
export async function getUser(wallet_address_id: string, callback: (r: Users | undefined) => void) {
    dbcon.query(`SELECT wallet_address_id,name,role,nonce FROM RFDATA.USERS WHERE LOWER(wallet_address_id) = LOWER("${wallet_address_id}")`, function (err, result, fields) {
        if (err)
            throw err;
        const rows = <RowDataPacket[]>result;
        ...
        if (rows.length > 0)
            record = {
                "wallet_address_id": rows[0].wallet_address_id, "name": rows[0].name, "role": rows[0].role, "nonce": rows[0].nonce
            };
        else
            record = undefined
        callback(record);
...
export async function getUserByPublicKey(publicKey: string): Promise<Users | undefined> {
    return new Promise((resolve, reject) => {
        dbcon.query(`SELECT wallet_address_id,name,role,nonce FROM RFDATA.USERS WHERE LOWER(wallet_address_id) = LOWER("${publicKey}")`, function (err, result, fields) {
            if (err) {
                reject(err);
                return;
            }
...
export async function updateUserNonce(wallet_address_id: string) {
    ...
    const nonce = Math.floor(Math.random() * 1000000);
    dbcon.query(
        `UPDATE RFDATA.USERS SET nonce = ?,updated_on = FROM_UNIXTIME(? * 0.001) WHERE LOWER(wallet_address_id) = LOWER("${wallet_address_id}")`,
...
export async function updateUser(wallet_address_id: string, data: Users) {
    let now = Date.now();
    dbcon.query(
        `UPDATE RFDATA.USERS SET wallet_address_id = ?,name = ? ,role = ?,updated_on = FROM_UNIXTIME(? * 0.001) WHERE LOWER(wallet_address_id) = LOWER(${wallet_address_id}")`,
        [data["wallet_address_id"], data["name"], data["role"], now]
    );
    data["updated_on"] = now;
    return data;
}
...
export async function deleteUser(wallet_address_id: string) {
    dbcon.query(`DELETE FROM RFDATA.USERS WHERE LOWER(wallet_address_id) = LOWER(${wallet_address_id}")`);
