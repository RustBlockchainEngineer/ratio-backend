...
import {
  DevnetCoinGeckoTokenMints,
  MainnetCoinGeckoTokenMints,
} from "../models/model";
...
export async function getSaberPrice() {
  const price = await axios.get(
    `${COINGECKO_API}simple/price?ids=saber&vs_currencies=usd`
  );
  return price.data.saber.usd;
}

const CoinGeckoTokenList = DEVNET
  ? DevnetCoinGeckoTokenMints
  : MainnetCoinGeckoTokenMints;
...
export async function saveCoinGeckoPrices() {
  ...
  for (const token in CoinGeckoTokenList) {
    const data = await axios.get(
      `${COINGECKO_API}simple/price?ids=${CoinGeckoTokenList[token]}&vs_currencies=usd`
    );
  ...
  for (const t of Object.keys(tokenPriceList)) {
    dbcon.query(
      `INSERT INTO RFDATA.TOKENPRICES(
      token,
      price,
      created_on,
      source)
  ...
  for (const pool of saberLPoolList) {
    dbcon.query(
      `INSERT INTO RFDATA.SOURCELPPRICES(
      pool_name,
      lp_price,
      tokenasize,
      tokenbsize,
